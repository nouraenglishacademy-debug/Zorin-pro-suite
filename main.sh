...
mkdir -p /tmp/zorin_build/DEBIAN /tmp/zorin_build/usr/bin /tmp/zorin_build/usr/share/applications && cat << 'EOF' > /tmp/zorin_build/DEBIAN/control
...
EOF
cat << 'EOF' > /tmp/zorin_build/usr/bin/zorin-pro-suite
#!/bin/bash
...
EOF
chmod 755 /tmp/zorin_build/usr/bin/zorin-pro-suite
cat << 'EOF' > /tmp/zorin_build/usr/share/applications/zorin-pro-suite.desktop
[Desktop Entry]
Version=1.0
Type=Application
...
StartupNotify=true
EOF
chmod 644 /tmp/zorin_build/usr/share/applications/zorin-pro-suite.desktop
dpkg-deb --build /tmp/zorin_build ./zorin-pro-suite_1.0.1_all.deb && rm -rf /tmp/zorin_build && echo "✅ تم الإصلاح والبناء بنجاح! الملف الجاهز والمستقر: zorin-pro-suite_1.0.1_all.deb"
